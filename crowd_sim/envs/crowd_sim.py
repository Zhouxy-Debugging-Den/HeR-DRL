...
class CrowdSim(gym.Env):
    ...
    def render(self, mode='video', output_file=None):
        ...
        robot_color = 'c'
        arrow_style = patches.ArrowStyle.Fancy(head_length=10, head_width=4, tail_width=.6)
        ...
        if mode == 'traj':
            ...
            for k in range(len(self.states)):
                if k % 12 == 0 or k == len(self.states) - 1:
                    ...
                    humans = [plt.Circle(human_positions[k][i], self.humans[i].radius, fill=False, color=human_colors[i])
                              for i in range(len(self.humans))]
                    ax.add_artist(robot)
                    for human in humans:
                        ax.add_artist(human)
            ...
            ax.legend([robot,human], ['Robot','Human'], fontsize=19.5,loc='upper center', ncol=2)
            ...
            if len(self.humans) == 0:
                ...
                humans = [plt.Circle(human_positions[0][i], self.humans[i].radius, fill=False, color=human_colors[i],lw=1.5)
                          for i in range(len(self.humans))]
                plt.legend([robot, humans[0]], ['Robot', 'Human'], fontsize=15,loc='upper center', ncol=2)
