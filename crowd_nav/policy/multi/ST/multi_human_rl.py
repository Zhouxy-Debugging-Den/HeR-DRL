...
from crowd_nav.policy.multi.cadrl import CADRL
...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state):
        ...
        if not state[len(state)-1].agent_states:
            ...
            for action in self.action_space:
                ...
                if self.query_env:
                    ...
                    next_agent_states = [self.propagate(agent_state, ActionXY(agent_state.vx, agent_state.vy))
                                         for agent_state in state[len(state)-1].agent_states]
                ...
                if batch_tensor is None:
                    for i in range(len(state) - 1):
                        mid_states = torch.cat([torch.Tensor([state[i + 1].robot_state + agent_state]).to(self.device)
                                                for agent_state in state[i + 1].agent_states], dim=0)
        ...
        self.last_state = state[len(state) - 1].agent_states
    ...
    def transform(self, state):
        ...
        """
        state_tensor = torch.cat([torch.Tensor([state.robot_state + agent_state]).to(self.device)
