...
import numpy as np
import logging
from crowd_sim.envs.policy.single.policy import Policy
...
class CADRL(Policy):
    def __init__(self):
        ...
        self.joint_state_dim = None
        self.multi_carto=True
        # self.multi_carto = False
        if self.multi_carto:
            self.agent_state_dim = 8
        else:
            self.agent_state_dim = 7
        self.joint_state_dim = self.self_state_dim + self.agent_state_dim
    ...
    def propagate_more(self, state, action):
        ...
        if isinstance(state, ObservableState):
            ...
            next_state = ObservableState(next_px, next_py, action.vx, action.vy, state.radius,state.multi)
            ...
            if self.kinematics == 'holonomic':
                next_px = state.px + action.vx * self.time_step * 2
                next_py = state.py + action.vy * self.time_step * 2
                next_state = FullState(next_px, next_py, action.vx, action.vy, state.radius,state.catogory,
                ...
                next_state = FullState(next_px, next_py, next_vx, next_vy, state.radius, state.catogory,
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            ...
            if self.kinematics == 'holonomic':
                next_px = state.px + action.vx * self.time_step
                next_py = state.py + action.vy * self.time_step
                next_state = FullState(next_px, next_py, action.vx, action.vy, state.radius,state.multi,
                                       state.gx, state.gy, state.v_pref, state.theta)
            else:
                next_theta = state.theta + action.r
                next_vx = action.v * np.cos(next_theta)
    ...
    def rotate(self, state):
        ...
        if self.multi_carto:
            new_state = torch.cat([dg, v_pref, theta, radius, vx, vy, px1, py1, vx1, vy1, radius1, da, radius_sum,agent_multi], dim=1)
        else:
